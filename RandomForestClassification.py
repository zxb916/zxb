...
def baggingDataSet(dataSet):
    n, m = dataSet.shape
    features = random.sample(dataSet.columns.values.tolist()[:-1], int(math.sqrt(m - 1)))
...
def testWine():
    ...
    df = pd.DataFrame(pd.read_csv('wine.txt', header=None))
    ...
    for i in range(tree_counts):
        ...
        decision_tree = createTree(bagging_data, bagginglabels)
    ...
    for tree in tree_list:
        ...
        label = treeClassify(tree, labels[:-1], test_data)
        label_pred.append(label)
    # 投票选择最终类别
    label_dict = {}
    for label in label_pred:
        label_dict[label] = label_dict.get(label, 0) + 1
    sort_class = sorted(label_dict.items(), key=lambda item: item[1])
    print("The predicted label is: {}".format(sort_class[-1][0]))
